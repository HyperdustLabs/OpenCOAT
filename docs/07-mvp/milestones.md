...
| **M6 — Heartbeat + Meta** | Decay / conflict / merge / archive / meta-review workers | Hermetic soak + daemon heartbeat maintenance ship on `main`; optional 24h live soak for convergence metrics. Activation-time conflicts stay in `ConflictResolver`; background edges via `ConflictScannerWorker`. | ✅ — see [`README.md`](../../README.md) M6 row; [`m6-prerequisites-status.md`](./m6-prerequisites-status.md) |
