...
后续 (architecture (ii) / 研究向):

0. ~~文档对齐 + queue dogfood 收尾~~ ✅
1. ~~效应器 TCB 原型 + `PlasticityEngine` reweight~~ ✅ (warm + cold lift/archive)
2. ~~`r_t` JSONL + daemon 消费~~ ✅
3. **connectome split** 完整实现 (形式纲要 §5; 今仅有 cold lift `reflex:true` + archive)
4. ~~JSONL replay 可塑性单测~~ ✅
5. **EffectorKernel.run_turn** 全回合闭环 (§3.5 — 今为 weaving + 局部 TCB)
6. **24h live soak** 运维验收 (`examples/07_meta_governance_soak`)
