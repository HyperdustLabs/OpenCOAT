# ADR 0010 — Concern unit, AspectJ executable syntax
...
the Concern Graph should speak **AspectJ vocabulary** rather than inventing
...
2. **Executable shape** on each Concern uses AspectJ lists:
...
3. **Legacy fields** `pointcut`, `advice`, `weaving_policy` remain valid; the
   protocol normalizes legacy ↔ AspectJ on load via
...
- New examples and bridge docs should show AspectJ-shaped JSON where possible.
