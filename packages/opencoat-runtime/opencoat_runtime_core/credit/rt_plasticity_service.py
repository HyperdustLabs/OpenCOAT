...
from opencoat_runtime_core.credit.credit_field import CreditField, CreditTurnResult
...
class RtPlasticityService:
    ...
    baseline_ema_alpha: float | None = None
    ...
    _turn_active: dict[str, list[ActiveAspect]] = field(default_factory=dict, repr=False)
    last_credit: CreditTurnResult | None = field(default=None, repr=False)
    ...
    def __post_init__(self) -> None:
        ...
        self._lifecycle = self.lifecycle or ConcernLifecycleManager(
            concern_store=self.concern_store,
            dcn_store=self.dcn_store,
        )
        baseline = RewardBaseline(ema_alpha=self.baseline_ema_alpha or 1.0)
        self._credit = CreditField(
            concern_store=self.concern_store,
            buffer=self.buffer,
            baseline=baseline,
        )
    ...
    @property
    def credit_field(self) -> CreditField:
        assert self._credit is not None
        return self._credit

    def record_coactivation(self, turn_key: str, concern_ids: list[str]) -> None:
        """Track aspects co-active in one weave (feeds connectome warm path)."""
        if not turn_key or not concern_ids:
            return
        bucket = self._turn_concerns.setdefault(turn_key, set())
        for cid in concern_ids:
            if cid and cid.strip():
                bucket.add(cid.strip())

    def record_turn_activations(
        self,
        turn_key: str,
        activations: list[ActiveAspect],
    ) -> None:
        """Paper §3: register ``a_i`` + hard/soft for tier-1 ``ρ`` at ``r_t`` time."""
        if not turn_key or not activations:
            return
        self._turn_active[turn_key] = list(activations)
        members = self._turn_concerns.setdefault(turn_key, set())
        for a in activations:
            members.add(a.concern_id)
    ...
    def append(self, record: RtRecord) -> dict[str, Any]:
        ...
        active = self._turn_active.pop(turn, None)
        result = self._credit.attribute_turn(normalized, active=active)
        self.last_credit = result
        payload = dict(normalized.signal.payload or {})
        payload["advantage"] = result.advantage
        normalized = normalized.model_copy(
            update={
                "baseline_b": result.baseline_b,
                "signal": normalized.signal.model_copy(update={"payload": payload}),
            }
        )
        members = {a.concern_id for a in result.aspect_attributions}
        if members:
            self._turn_concerns.setdefault(turn, set()).update(members)
    ...
    def consume(self, *, max_records: int | None = None) -> WarmStepStats:
        ...
        with self._consume_lock:
            ...
            assert self._credit is not None
            ledger = self._credit.drain_synapse_ledger()
            ...
            ledger_stats = apply_synapse_kappa_ledger(
                ledger,
                concern_store=self.concern_store,
                dcn_store=self.dcn_store,
            )
            warm = warm.__class__(
                ...
                connected=warm.connected,
                pruned=warm.pruned,
                skipped=warm.skipped,
                synapses_strengthened=warm.synapses_strengthened
                + int(ledger_stats.get("synapses_strengthened", 0)),
                synapses_weakened=warm.synapses_weakened
                + int(ledger_stats.get("synapses_weakened", 0)),
            )
            self.last_warm = warm
    ...
    def connectome_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "edges_by_relation": buckets,
            "reflex_core": sorted(view.reflex_core),
            "buffer_concerns": len(self.buffer.tracked_concern_ids()),
            "eligibility": self._credit.eligibility.snapshot() if self._credit else {},
            "last_conservation_residual": (
                self.last_credit.conservation_residual if self.last_credit else None
            ),
        }
